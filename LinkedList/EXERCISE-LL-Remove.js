...
class LinkedList {
  ...
  remove(index) {
    ...
    if (index < 0 || index >= this.length) return undefined;
    // Remove head using shift if index is 0
    if (index === 0) return this.shift();
    // Remove tail using pop if index is at (length - 1)
    if (index === this.length - 1) return this.pop();
    // Retrieve the element at (index - 1)
    const before = this.get(index - 1);
    // Store the node to be removed (before's next node)
    const temp = before.next;
    // Update before's next to point to the node after temp
    before.next = temp.next;
    // Set temp's next to null, removing reference
    temp.next = null;
